...
def infer_from_imgpath(imagen: str, net):
    out_path = "result"
    confianza = 0.5
    umbral = 0.1
    inference_width = 416
    ...
    cv2.imwrite(os.path.join(out_path, os.path.basename(imagen)), image)
    #serialized = pickle.dumps(idxs).decode('latin-1')
